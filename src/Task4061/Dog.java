    ...
    public Sayable toSayable(final int i) {
        class DogPet extends SuperDog implements Sayable {
            private String getName() {
                return new StringBuilder().append(getSuperQuotes()).append(Dog.this.name).append(getSuperQuotes()).toString();
            }
            ...
            public String say() {
                ...
                if (i < 1) {
                    result.append(String.format("%s спит.", getName()));
                } else {
                    StringBuilder aas = new StringBuilder();
                    for (int j = 0; j < i; j++) {
                        aas.append("а");
                    }
                    result.append(String.format("%s лает г%sв! %s", getName(), aas, formatter.format(new Date())));
                }
